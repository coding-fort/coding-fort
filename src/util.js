...
function generateNavInfo(dirInfo) {
  ...
  temp.nav[0].items = children
    .map((item) => {
      return { text: item.name, link: `${base}/${name}/${item.name}/` };
    })
    .filter((item) => item.text?.toLowerCase().indexOf("readme") == -1);
  // sidebar
  temp.sidebar = children.reduce((def, item) => {
    let key = `${base}/${name}/${item.name}`;
    def[key] = [
      {
        title: item.name,
        // collapsable: true,
        children: generateSidebar(item, key),
      },
    ];
    return def;
  }, {});
    ...
    if (children?.length > 0) {
      _temp = children
        .map((item) => {
          ...
          let _item = { title: name };
          // console.log("_item:::", _item.title);
          if (item.children) {
            // console.log(item.name);
            _item.children = generateSidebar(item, `${base}/${name}`);
            // console.log("_children:", _children);
          } else if (item.name.indexOf(".") > -1) {
            _item.path = `${base}/${name}`;
          }
          return _item;
        })
